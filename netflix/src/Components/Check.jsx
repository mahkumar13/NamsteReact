...
import { API_OPTION, IMAGE_CDN } from '../Utils/constant';
import { addCast, addMovieDetail, addTrailer, addTrailers } from '../Utils/movieSlice';
...
const Check = () => {
    ...
    // const {id}= useParams();
    // console.log("useparams",id)    // insted of using useSearchParams we can also use useParams hook 
    ...
    const getTrailer= async()=>{
        ...
        const trailer= filterData.lenghth?filterData[0]:json.results[0]
    ...
    const getDetails = async () => {
      ...
      const getCastDetail = async()=>{
        const data = await fetch("https://api.themoviedb.org/3/movie/"+ searchParams.get("v")+"/credits",API_OPTION)
        const json = await data.json()
  ...
  return (
   ...
   <div className='bg-black text-white'>
      <iframe className="w-full aspect-video h-[300px   ]"
       src={
    ...
    trailerData?.key
      ...
      <div className='md:flex p-[30px]'>
       ...
       <img className='w-56 mb-5 md:mb-0 rounded-lg  md:w-[550px] md:h-[700px] justify-center align-middle m-auto'
         src={IMAGE_CDN+MovieDetail?.poster_path}
         alt="backgroundimage"
       >
       </img>
        </div>
              ...
              {Cast?.cast.map((item) => {
                ...
                return(
                  <div  className="" key={item.id}>
                    <div >
                      <div className="p-3  w-[100px]"><img className="h-[85px] rounded-full" alt="profile_image" src={IMAGE_CDN + item.profile_path}/></div>
